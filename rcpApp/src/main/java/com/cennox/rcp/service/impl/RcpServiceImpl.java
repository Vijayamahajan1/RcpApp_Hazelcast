...
public class RcpServiceImpl implements RcpService {
    ...
    private final  IMap<Long, Acquirer> acquirerCache;

    private final IMap<Long, Device> deviceCache;
    ...
    public RcpServiceImpl(DeviceRepository deviceRepository, AcquirerRepository acquirerRepository , HazelcastInstance hazelcastInstance) {
        ...
        this.acquirerCache = hazelcastInstance.getMap("acquirer");
        this.deviceCache = hazelcastInstance.getMap("device");
    }
    ...
    List<Acquirer> acquirer = acquirerRepository.findAll();
         for (Acquirer acq : acquirer) {
            acquirerCache.put(acq.getCacheId(), acq);
        }
    }
    ...
    List<Device> device = deviceRepository.findAll();
         for (Device dev : device) {
            deviceCache.put(dev.getCacheId(), dev);
        }
    }
    ...
    public Device createDevice(Device device) {
        deviceCache.put(device.getCacheId(), device);
        return device;
    }

    @Override
    public Device getDeviceById(Long cacheId) {
        return deviceCache.get(cacheId);
    }
    ...
    public Acquirer createAcquirer(Acquirer acquirer) {
        acquirerCache.put(acquirer.getCacheId(), acquirer);
         return acquirer;
    }
    ...
    public Acquirer getAcquirerById(Long cacheId) {
        return acquirerCache.get(cacheId);
         
    }

    @Override
    public List<Acquirer> getAllAcquirers() {
        return new ArrayList<>(acquirerCache.values());
    }

    @Override
    public void deleteAcquirer(Long cacheId) {
        acquirerCache.remove(cacheId);
    }
